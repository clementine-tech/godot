...
# Compile Godot for Windows x64 with mono
scons platform=windows arch=x86_64 module_mono_enabled=yes precision=double production=yes
...
if dotnet nuget list source | grep -A 1 "[[:space:]]${NUGET_SOURCE_NAME}[[:space:]]\["; then
    ...
    VOXEL_SOURCE_PATH=$(dotnet nuget list source | grep -A 1 "[[:space:]]${NUGET_SOURCE_NAME}[[:space:]]\[" | tail -n 1 | sed 's/\\/\//g' | xargs)
